    ...
    module Net
      module HTTPAdapter
        ...
        def request method, uri, options = {}
          ...
          request.basic_auth(*[Invoicera.api_key, nil].flatten[0, 2])
          if options[:body]
          ...
          if Invoicera.logger
            Invoicera.log :info, "===> %s %s" % [request.method, uri]
            headers = request.to_hash
            headers['authorization'] &&= ['Basic [FILTERED]']
            Invoicera.log :debug, headers.inspect
            if request.body && !request.body.empty?
              Invoicera.log :debug, XML.filter(request.body)
            end
            start_time = Time.now
          end
          ...
          if Invoicera.logger
            ...
            Invoicera.log :debug, response.to_hash.inspect
            Invoicera.log :debug, response.body if response.body
          end
